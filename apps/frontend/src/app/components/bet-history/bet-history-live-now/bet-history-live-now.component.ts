...
@Component({
  selector: 'workspace-bet-history-live-now',
  templateUrl: './bet-history-live-now.component.html',
  styleUrls: ['./bet-history-live-now.component.scss']
})
export class BetHistoryLiveNowComponent implements OnInit, OnChanges, OnDestroy {

  @Input() liveNowData: any;
  cashingOutIndex: any;
    ...
    this.websocket.getData().subscribe(data => {
      if (data.data && data.data !== 'null' && data.data !== 'undefined') {
        if (data.rid === `cashingOut${this.websocketTimestamp}`) {
          this.liveNowData[this.cashingOutIndex].cashingOut = false;
          this.liveNowData[this.cashingOutIndex].cashedOut = true;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['liveNowData'] && changes['liveNowData'].currentValue) {
      this.liveNowData = changes['liveNowData'].currentValue;
      this.liveNowData.map((e,i) => {
        if(i <= 4){
  ...
  calculateCashOutAmount(e,  i){
    ...
    this.liveNowData[i]['calculatedCashout'] = price;
  }
  ...
  getCloseHistory(e, index){
    if(e){
      this.liveNowData[index].showHistory = false;
    }
  }
  formatAmount(amount){
  ...
  closePlayer(){
    this.player.dispose();
    this.player = undefined;
    this.liveNowData.map(e => e.showMobileEvent = false);
  }
  ngOnDestroy() {
    if(this.player){
      this.player.dispose();
