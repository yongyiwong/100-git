...
@Component({
  selector: 'workspace-bet-history-all',
  templateUrl: './bet-history-all.component.html',
  styleUrls: ['./bet-history-all.component.scss']
})
export class BetHistoryAllComponent implements OnInit, OnChanges, OnDestroy {
    ...
    this.websocket.getData().subscribe(data => {
      if (data.data && data.data !== 'null' && data.data !== 'undefined') {
        if (data.rid === `cashingOut${this.websocketTimestamp}`) {
          this.allData[this.cashingOutIndex].cashingOut = false;
          this.allData[this.cashingOutIndex].cashedOut = true;
          this.subscriptionsService.setIfNewBetHistory(true);
        }
      }
    })
    this.allData.map((e,i) => {
      if(i <= 4){
  ...
  ngOnDestroy() {
    if(this.player){
      ...
      this.allData.map(e => e.showMobileEvent = false);
    }
  }
  ...
  calculateCashOutAmount(e,  i){
    ...
    this.allData[i]['calculatedCashout'] = price;
  }
  ...
  getCloseHistory(e, index){
    if(e){
      this.allData[index].showHistory = false;
    }
  }
  formatAmount(amount){
